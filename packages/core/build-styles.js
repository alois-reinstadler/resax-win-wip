...
const main = async () => {
    ...
    console.log(pattern, filepaths, styles);
    if (!styles.length) return console.warn`No stylesheets found. Skipping CSS generation.`;

    console.log(`Found ${styles.length} for compilation`);
    for (const style of styles) {
        ...
        // await emit(path, content);
        // sourcemap && (await emit(path + '.map', sourcemap));
        let content = '';
        let prevMap;
        ...
        if (content) {
            if (!isSassModule) {
                const result = await processCss({ content, name, path: newPath, prev: prevMap });
                content = result.css;

                if (content && result.map) {
                    content += `/*# sourceMappingURL=${encodeURIComponent(newFile + '.map')} */`;
...
function toStyleInfo(...filepaths) {
    ...
    for (const stylepath of filepaths) {
        ...
        const relPath = segments.filter(s => !rootSegments.includes(s)).join('/');
